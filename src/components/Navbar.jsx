...
function Navbar(){
    ...
    return (
        <section className="navbar-section sm:p-3 text-[--color-1] flex items-center justify-between">
            ...
            <button aria-controls="#navbar-list" aria-expanded="false"
                onClick={navTogglefunction} ref={navToggle}
                className='nav-toggle-button aspect-square top-7 right-8 [@media(max-width:40em)]:block hidden'
            >
                <span className={ `${isOpen ?  'hidden' : 'visible'}` }><RxHamburgerMenu size={30}/></span>
                <span className={ `${isOpen ?  'visible' : 'hidden'}` }><AiOutlineClose size={30}/></span>
            </button>

            <nav>
                <ul id="navbar-list" className="flex gap-5 sm:py-8 sm:px-16 navbar-list" data-visible="false" ref={navList}>
                    ...
                    <li className='hover:-translate-y-1 transition-all hover:shadow-xl rounded-lg p-2 ease-linear'>
                        <a  href="#">
                            <span aria-hidden ="true">03</span>EDUCATION
                        </a>
                    </li>
                </ul>
            </nav>
